...
export default function BlackBlue({ resume }: { resume: Resume }) {
            ...
            <div>
              <div>
                ...
                <div className={styles.skills}>
                  {skills?.map(skill => <div key={skill.id} className={styles.skillItem}><div>{skill.name}</div>{skill.level > 0 && <SkillRate rate={skill.level} activeColor="var(--secondary)" color="rgba(255,255,255,.38)" />}</div>)}
                </div>
              </div>
            </div>
          }
