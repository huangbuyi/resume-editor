...
export default function HoneyBee({ resume }: { resume: Resume }) {
    ...
    <div className={styles.main}>
      <div className={styles.sider}>
          ...
          <div className={styles.block}>
            <div>
              ...
              <div className={styles.skills}>
                {skills?.map(skill => <div key={skill.id} className={styles.skillItem}><div>{skill.name}</div>{skill.level > 0 && <SkillRate rate={skill.level} activeColor="var(--secondary)" color="#fff" />}</div>)}
              </div>
            </div>
          </div>
        }
        <div className={styles.divider2}></div>
        {
          interests.length > 0 &&
          <div>
            <h2 className={styles.interestHead}>兴趣爱好</h2>
