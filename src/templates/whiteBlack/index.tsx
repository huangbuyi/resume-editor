...
import image from './geomety-circle.svg';

export default function WhiteBlack({ resume }: { resume: Resume }) {
  const { name, title, infos, skills, experiences, educations, projects, interests, introduction } = resume;
  return <div className={styles.root}>
    <div className={styles.main}>
      <div className={styles.sider}>
        <img className={styles.decorateImg} src={image} />
      ...
      <div className={styles.siderPlaceholder}></div>
            ...
            <div className={styles.block}>
              <div>
                ...
                <div className={styles.skills}>
                  {skills?.map(skill => <div key={skill.id} className={styles.skillItem}><div>{skill.name}</div>{skill.level > 0 && <SkillRate rate={skill.level} activeColor="var(--primary)" color="rgba(255,255,255,.38)" />}</div>)}
                </div>
              </div>
            </div>
          }
