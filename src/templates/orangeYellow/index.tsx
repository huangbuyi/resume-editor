...
export default function OrangeYellow({ resume }: { resume: Resume }) {
            ...
            <div className={styles.block}>
              <div>
                ...
                <div className={styles.skills}>
                  {skills?.map(skill => <div key={skill.id} className={styles.skillItem}><div>{skill.name}</div>{skill.level > 0 && <SkillRate rate={skill.level} activeColor="var(--primary)" color="var(--secondary)" />}</div>)}
                </div>
              </div>
            </div>
          }
